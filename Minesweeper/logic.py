...
class Minesweeper:
    ...
    def place_mines(self):
        ...
        while count < self.mines:
            ...
            # Prevent placing multiple mines in the same cell
            if self.board[r][c] != "M":
                self.board[r][c] = "M"
                count += 1

    def calculate_numbers(self):
        # Fill each non-mine cell with count of adjacent mines
    ...
    def flood_fill(self, r, c):
        ...
        # Only expand if the cell has no adjacent mines
        if self.board[r][c] == "0":
            for i in range(r - 1, r + 2):
                for j in range(c - 1, c + 2):
                    if i != r or j != c:
